...
int nvme_mi_mi_ctrl_health_status_poll(nvme_mi_ep_t ep,
					    struct nvme_mi_ctrl_health_status *chs)
{
	...
	req_hdr.hdr.nmp = (NVME_MI_ROR_REQ << 7) |
		(NVME_MI_MT_MI << 3);
	req_hdr.opcode = nvme_mi_mi_opcode_ctrl_health_status_poll;
	req_hdr.cdw0 = 1 << 31;
	...
	nvme_msg (ep->root, LOG_INFO, "MI ctrl data_len: %d\n", resp.data_len);

	if (resp.data_len != sizeof(*chs)) {
		nvme_msg(ep->root, LOG_WARNING,
			 "MI Subsystem Health Status length mismatch: "
			 "got %zd bytes, expected %zd\n",
			 resp.data_len, sizeof(*chs));
